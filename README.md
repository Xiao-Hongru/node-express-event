...
1. 在 `my_db_01` 数据库中，新建 `ev_users` 表如下：
   - id 类型:INT
   - username 类型：VARCHAR(255)
   - password 类型：VARCHAR(255)
   - nickname 类型：VARCHAR(255)
   - email 类型：VARCHAR(255)
   - user_pic 类型：VARCHAR(255)
...
- id 类型：INT
- name 类型：VARCHAR(255)
- alias 类型：VARCHAR(255)
- is_delete 类型：TINYINT(1) 默认0
...
- id 类型：INT
- title 类型：VARCHAR(255)
- content 类型：TEXT
- cover_img 类型：VARCHAR(255)
- pub_date 类型：VARCHAR(255)
- state 类型：VARCHAR(255)
- is_delete 类型：TINYINT(1)
- cate_id 类型：INT
- author_id 类型：INT
